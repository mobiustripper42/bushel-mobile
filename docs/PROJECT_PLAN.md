...
| # | Task | Pts | Status |
|---|---|---|---|
| 1.1 | Email-code login (request code → exchange for bearer, store in `expo-secure-store`) + read-only active-orders screen | 5 | |
| 1.2 | `expo-notifications` + Expo push API; register device token to bushel; order-arrival fan-out to Annabel. **v1 = Android-only push** (iOS remote push deferred behind the $99 Apple Dev gate) | 5 | |
| 1.3 | Mark-fulfilled mutation — one write path through bushel 11.1 `/api/*`; optimistic UI. Exactly one mutation (rehearsal, not an admin port) | 2 | |
| 1.4 | Android EAS build → sideloaded APK for Annabel ($0, no Mac). Emma's iPhone: free-signed 7-day build, install-only; document the iOS-push-$99 gate + the 7-day re-sign reality | 3 | |
